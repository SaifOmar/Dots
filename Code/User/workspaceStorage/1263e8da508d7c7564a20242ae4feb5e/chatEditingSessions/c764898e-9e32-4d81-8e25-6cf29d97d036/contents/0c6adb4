  ...
  <div class="min-h-screen bg-gradient-genius-hero font-cairo">
    ...
    <section class="bg-gradient-to-br from-genius-gray-900/90 via-genius-purple-900/80 to-genius-blue-600/90 relative overflow-hidden py-20">
      ...
      <div class="absolute inset-0 overflow-hidden">
        ...
        <div class="absolute bottom-0 right-1/4 w-64 h-64 bg-genius-pink-400/10 rounded-full animate-pulse" style="animation-delay: 2s;"></div>
      </div>
    ...
    <section class="py-8 bg-genius-gray-800/50 backdrop-blur-sm">
      <div class="container mx-auto px-4">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div class="md:col-span-2">
            <div class="relative">
              <input
                ...
                class="w-full px-6 py-4 bg-genius-gray-900/50 border border-genius-gray-700 rounded-xl text-genius-white placeholder-genius-gray-400 focus:border-genius-blue-500 focus:ring-1 focus:ring-genius-blue-500 transition-all duration-300"
              >
          ...
          <div>
            <select
              ...
              class="w-full px-6 py-4 bg-genius-gray-900/50 border border-genius-gray-700 rounded-xl text-genius-white appearance-none cursor-pointer focus:border-genius-blue-500 focus:ring-1 focus:ring-genius-blue-500 transition-all duration-300"
            >
        ...
        <div v-if="hasActiveFilters" class="mt-6 flex flex-wrap gap-3">
          <div v-if="searchQuery" 
               class="px-4 py-2 bg-genius-blue-500/20 border border-genius-blue-500 rounded-lg text-genius-blue-400 text-sm flex items-center">
            <span>{{ searchQuery }}</span>
            <button @click="searchQuery = ''" class="ml-2">×</button>
          </div>
          <div v-if="selectedLevel" 
               class="px-4 py-2 bg-genius-purple-500/20 border border-genius-purple-500 rounded-lg text-genius-purple-400 text-sm flex items-center">
            <span>{{ getLevelName(selectedLevel) }}</span>
            <button @click="selectedLevel = ''" class="ml-2">×</button>
          </div>
          <div v-if="selectedSubject" 
               class="px-4 py-2 bg-genius-pink-500/20 border border-genius-pink-500 rounded-lg text-genius-pink-400 text-sm flex items-center">
            <span>{{ getSubjectName(selectedSubject) }}</span>
            <button @click="selectedSubject = ''" class="ml-2">×</button>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="py-20 relative">
      ...
      <div class="container mx-auto px-4 relative z-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          <template v-if="isLoading">
            <div v-for="i in 6" :key="i" class="animate-pulse">
              <div class="bg-genius-gray-800/80 rounded-2xl overflow-hidden">
                <div class="h-64 bg-genius-gray-700"></div>
                <div class="p-6">
                  <div class="flex items-center mb-4">
                    <div class="w-12 h-12 bg-genius-gray-700 rounded-full"></div>
                    <div class="flex-1 ml-3">
                      <div class="h-4 bg-genius-gray-700 rounded w-3/4"></div>
                      <div class="h-3 bg-genius-gray-700 rounded w-1/2 mt-2"></div>
                    </div>
                  </div>
                  <div class="h-4 bg-genius-gray-700 rounded w-1/4"></div>
                </div>
              </div>
            </div>
          </template>
        ...
        <!-- Empty State -->
        <div v-if="!isLoading && filteredCourses.length === 0" class="text-center py-16">
          <div class="w-24 h-24 bg-genius-gray-800 rounded-full mx-auto mb-6 flex items-center justify-center">
            <svg class="w-12 h-12 text-genius-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M12 12h.01M12 12h.01M12 12h.01M12 12h.01M12 12h.01M12 12h.01M12 12h.01M12 12h.01M12 12h.01M12 12h.01M12 12h.01"></path>
            </svg>
          </div>
          <h3 class="text-2xl font-bold text-genius-white mb-2">لا توجد نتائج</h3>
          <p class="text-genius-gray-400">جرب تغيير معايير البحث أو تصفية النتائج</p>
        </div>
...
// Filter options (these would come from your backend)
const levelOptions = [
...
const filteredCourses = computed(() => {
  return props.courses.filter(course => {
    const matchesLevel = selectedLevel.value ? course.level_id === selectedLevel.value : true
    const matchesSubject = selectedSubject.value ? course.subject_id === selectedSubject.value : true
...
const hasActiveFilters = computed(() => {
  return searchQuery.value || selectedLevel.value || selectedSubject.value
})
...
const filterCourses = () => {
  ...
  // Here you would typically make an API call to fetch filtered courses
...
const getLevelName = (levelId) => {
  return levelOptions.find(l => l.id === levelId)?.name || levelId
}

const getSubjectName = (subjectId) => {
  return subjectOptions.find(s => s.id === subjectId)?.name || subjectId
}
...
// Watchers
watch([selectedLevel, selectedSubject, searchQuery], () => {
  // Refine the course filtering whenever the filters change
  filterCourses()
})

// Watch for filter changes
watch([searchQuery, selectedLevel, selectedSubject], () => {
  performSearch()
})

// Debounced version of the filter function to limit API calls
